...
public class OrderService {
    ...
    public OrderEntity checkout(CheckoutRequest request, Principal principal) {
        ...
        order.setPaymentMethod(request.paymentMethod() == null || request.paymentMethod().isBlank() ? "cod" : request.paymentMethod());
        order.setPaymentStatus("bank".equalsIgnoreCase(order.getPaymentMethod()) ? "awaiting" : "pending");
        ...
        if (saved.getUser() != null) {
            ...
            try {
                if (saved.getUser().getEmail() != null && !saved.getUser().getEmail().isBlank()) {
                    mailService.sendOrderConfirmationMail(saved.getUser().getEmail(), saved, displayOrderNumber(saved));
                }
            } catch (Exception ignored) {}
        }
        cartService.clear();
    ...
    public long displayOrderNumber(OrderEntity order) {
        if (order == null) return 0L;
        if (order.getUser() == null || order.getUser().getId() == null) {
            return order.getId() != null ? order.getId() : 0L;
        }
        List<OrderEntity> scoped = orderRepository.findByUserOrderByCreatedAtDesc(order.getUser()).stream()
                .filter(o -> !isDeleted(o))
                .sorted(Comparator.comparing(OrderEntity::getId))
                .toList();
        for (int i = 0; i < scoped.size(); i++) {
            OrderEntity item = scoped.get(i);
            if (item != null && Objects.equals(item.getId(), order.getId())) {
                return i + 1L;
            }
        }
        return order.getId() != null ? order.getId() : 0L;
